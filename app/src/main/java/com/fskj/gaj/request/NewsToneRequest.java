...
public class NewsToneRequest extends BaseListRequest<NewsToneCommitVo,MsgSearchResultVo>{
    public NewsToneRequest(Context activeContext, NewsToneCommitVo requestData, ResultListInterface<MsgSearchResultVo> listener) {
        super(activeContext, requestData, listener);
    }

    @Override
    protected ResultTVO<MsgSearchResultVo> Query_Process() throws IOException, Exception {
        String url = BuildConfig.SERVER_IP + "/mobile/newestone.do";
