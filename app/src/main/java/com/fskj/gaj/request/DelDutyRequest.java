...
public class DelDutyRequest extends BaseObjRequest<DelDutyCommitVo,String> {
    public DelDutyRequest(Context activeContext, DelDutyCommitVo requestData, ResultObjInterface<String> listener) {
        super(activeContext, requestData, listener);
    }

    @Override
    protected ResultVO<String> Query_Process() throws IOException, Exception {
        String url = BuildConfig.SERVER_IP + "/mobile/delduty.do";
        String jsonStr = HttpUtils.post(url,requestData.toMap());
//        Log.e("DelDutyRequest",jsonStr);
